...
{
public:
	//ƒобавить конструктор перемещени€, конструктор удаленный копировани€ (старый объект нельз€ будет использовать) 
	CCanvas(SDL_Window* window);
	~CCanvas();
	CCanvas(CCanvas&& canvas);
	//ƒописать оператор перемещающего присваивани€ (cg-course)
